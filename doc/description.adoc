...
* enables all the operators `~`, `&`, `|`, `^`, `&=`, `|=`, `^=` for `E` and `complement<E>` (xref:_complementing_the_complement[see below])
...
Once, there was a kingdom, where the Queen of Hearts reigned with iron fist. She had her own newspaper for proclamations, where they used a set of flags for font-styles</span>+++
[source, C++]
----
enum font_styles {
    bold      = 1,
...
The language allows overloading when at least one of its arguments is of `class` or `enum` type
[source, C++]
----
enum E{};
bool operator < (E,E){ return true; }
bool test(){
    return E{} < E{};       // our operator < and returns true
}
----
This also works for function templates with the one exception: when an operator template is invoked with the same enumeration type for both arguments, then the built-in operator is called.
[source, C++]
----
enum E{};
...
https://www.open-std.org/jtc1/sc22/wg21/docs/cwg_active.html#2730
 which would make the later call the user defined operator template.
====

Currently, deleting (resp. overloading) relational operators requires the definition of a macro

* `BOOST_FLAGS_REL_OPS_DELETE(E)`: deletes all relational operators for enumeration `E`
* `BOOST_FLAGS_REL_OPS_PARTIAL_ORDER(E)`: defines all relational operators to reflect the partial order induced by `contained` (see below)
